...
class MovieListModel {
  ...
  factory MovieListModel.fromJson(Map<String, dynamic> json) => MovieListModel(
    ...
    movieGenres: List<MovieGenre>.from(json["movieGenres"].map((x) => MovieGenre.fromJson(x))),
...
class Comment {
  ...
  Map<String, dynamic> toJson() => {
    "id": id,
    "text": text,
    "commentDate": "${commentDate.year.toString().padLeft(4, '0')}-${commentDate.month.toString().padLeft(2, '0')}-${commentDate.day.toString().padLeft(2, '0')}",
  };
}
